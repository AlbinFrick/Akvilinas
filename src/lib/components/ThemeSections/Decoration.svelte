<script lang="ts">
  ...
  import { type InputValue, PortableText } from '@portabletext/svelte';

  export let decoration: Decoration;
  const decorationDescription = decoration.base
    .description as unknown as InputValue;
</script>
  ...
  <div class="mx-auto max-w-default-content space-y-6">
    ...
    <div class="flex flex-col gap-4 md:flex-row md:gap-10">
      <div>
        ...
        <p>
          Detta är dekorationer som inte ingår i basdekorationerna men ändå inte
          är en figur. Priset kommer vi överens med
        </p>
      </div>
      <div
        class="mx-auto grid grid-cols-2 overflow-clip rounded-xl *:m-0 *:aspect-square *:object-cover md:w-1/2"
